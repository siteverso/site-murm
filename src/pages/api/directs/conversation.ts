...
import { archiveConversation, deleteConversationForUser, restoreConversation } from '../../../lib/server/repositories/directs';
...
export const POST: APIRoute = async context => {
    try {
        ...
        if (!Number.isInteger(otherUserId) || otherUserId <= 0 || !['archive', 'restore', 'delete'].includes(action)) {
            throw new Error('DIRECT_INVALIDO');
        }

        if (action === 'archive') await archiveConversation(user.id, otherUserId);
        else if (action === 'restore') await restoreConversation(user.id, otherUserId);
