...
const themeCodes = ['pearl', 'graphite', 'graphite-bright', 'ocean', 'forest', 'sunset', 'rose', 'purple'];
...
test('theme picker exposes eight visual themes and saves immediately', async () => {
...
test('incremental Oracle patches preserve legacy migration and allow eight themes', async () => {
  ...
  const patch = await read('murm-oracle/database/patches/20260713-0230-add-graphite-bright-theme.sql');
  assert.match(patch, /CHECK \(theme_code IN \('pearl', 'graphite', 'graphite-bright', 'ocean', 'forest', 'sunset', 'rose', 'purple'\)\)/);
});
